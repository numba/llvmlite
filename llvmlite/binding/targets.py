...
# FIXME: subclassing built-ins isn't exactly great in < 3.9
class FeatureMap(dict):  # type: ignore
    """
    ...
    def flatten(self, sort: bool = True) -> str:
        ...
        """
        # FIXME: either list or iterator?
        iterator = sorted(self.items()) if sort else iter(self.items())  # type: ignore
        flag_map = {True: "+", False: "-"}
        return ",".join("{0}{1}".format(flag_map[v], k) for k, v in iterator)  # type: ignore
...
class TargetData(ffi.ObjectRef):
    ...
    def get_abi_size(self, ty: Any) -> int:
        ...
        """
        return ffi.lib.LLVMPY_ABISizeOfType(self, ty)  # type: ignore
    ...
    def get_pointee_abi_size(self, ty: TypeRef) -> int:
        ...
        """
        size = ffi.lib.LLVMPY_ABISizeOfElementType(self, ty)  # type: ignore
        if size == -1:
            raise RuntimeError(f"Not a pointer type: {ty}")
    ...
    def get_pointee_abi_alignment(self, ty: TypeRef) -> int:
        ...
        """
        size = ffi.lib.LLVMPY_ABIAlignmentOfElementType(self, ty)  # type: ignore
        if size == -1:
            raise RuntimeError(f"Not a pointer type: {ty}")
        return size  # type: ignore
