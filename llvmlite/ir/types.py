...
class PointerType(Type):
    ...
    def intrinsic_name(self) -> str:
        return f"p{self.addrspace}{self.pointee.intrinsic_name}"  # type: ignore
...
class VectorType(Type):
    ...
    def wrap_constant_value(self, values: Iterable[Value]) -> Iterable[Value]:  # type: ignore
        ...
        if len(values) != len(self):
            raise ValueError(
                "wrong constant size for {}: got {} elements".format(self, len(values))
            )
        return [
            Constant(ty, val) if not isinstance(val, Value) else val  # type: ignore
