...
class Instruction(NamedValue, _HasMetadata):
    ...
    def function(self) -> Function:
        ...
        return cast(Block, self.parent).function
    ...
    def descr(self, buf: list[str]) -> None:
        ...
        # FIXME: get_reference is unknown
...
class CallInstr(Instruction):
    def __init__(
        ...
        super(CallInstr, self).__init__(parent, func.function_type.return_type,
                                        "call", [func] + list(args), name=name)  # type: ignore
...
class Terminator(Instruction):
    ...
    def descr(self, buf: list[str]) -> None:
        opname = self.opname
        # FIXME: type and get_reference unknown
        operands = ', '.join(["{0} {1}".format(op.type, op.get_reference())  # type: ignore
                              for op in self.operands])
        metadata = self._stringify_metadata(leading_comma=True)
...
class SwitchInstr(PredictableInstr, Terminator):
    ...
    def descr(self, buf: list[str]) -> None:
        cases = ["{0} {1}, label {2}".format(val.type, val.get_reference(),  # type: ignore
                                             blk.get_reference())
...
class GEPInstr(Instruction):
    def __init__(
        ...
        super(GEPInstr, self).__init__(parent, typ, "getelementptr",
                                       [ptr] + list(indices), name=name)  # type: ignore
        self.pointer = ptr
...
class InsertElement(Instruction):
    ...
    def descr(self, buf: list[str]) -> None:
        operands = ", ".join("{0} {1}".format(
                   op.type, op.get_reference()) for op in self.operands)  # type: ignore
        buf.append("{opname} {operands}\n".format(
                   opname=self.opname, operands=operands))
...
class ShuffleVector(Instruction):
    ...
    def descr(self, buf: list[str]) -> None:
        buf.append("shufflevector {0} {1}\n".format(
                   ", ".join("{0} {1}".format(op.type, op.get_reference())  # type: ignore
                             for op in self.operands),
                   self._stringify_metadata(leading_comma=True),
                   ))
...
class CmpXchg(Instruction):
    ...
    def __init__(
        ...
        super(CmpXchg, self).__init__(parent, outtype, "cmpxchg",
                                      (ptr, cmp, val), name=name)  # type: ignore
        self.ordering = ordering
...
class Fence(Instruction):
    ...
    def __init__(
        self,
        parent: Block,
        ordering: str,  # Literal["acquire", "release", "acq_rel", "seq_cst"],
