...
class _ConstOpMixin(object):
    ...
    def ptrtoint(self, typ: types.Type) -> Constant:
        ...
        if not isinstance(self.type, types.PointerType):  # type: ignore
            ...
            raise TypeError(msg % (self.type,))  # type: ignore
...
class GlobalValue(NamedValue, _ConstOpMixin, _HasMetadata):
    ...
    def __init__(
        ...
        super(GlobalValue, self).__init__(parent=parent, type=type, name=name)
        self.linkage = ''
        self.storage_class = ''
        self.section = ''
...
class Function(GlobalValue):
    ...
    def descr_prototype(self, buf: list[str]) -> None:
        ...
        attrs = ' {}'.format(_attrs) if _attrs else ''
        if any(self.args):
...
class ArgumentAttributes(AttributeSet):
    _known = frozenset(['byval', 'inalloca', 'inreg', 'nest', 'noalias',  # type: ignore
...
class Block(NamedValue):
    ...
    def __init__(self, parent: Function, name: str = "") -> None:
        super(Block, self).__init__(parent=parent, type=types.LabelType(), name=name)
