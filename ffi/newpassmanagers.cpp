...
LLVMPY_CreateNewModulePassManager() { return llvm::wrap(new PassManager<Module>()); }
...
API_EXPORT(void)
LLVMPY_AddAAEvalPassNMPM(LLVMModulePassManagerRef MPM) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(AAEvaluator()));
}

API_EXPORT(void)
LLVMPY_AddSimplifyCFGPass_module(LLVMModulePassManagerRef MPM) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(SimplifyCFGPass()));
}

API_EXPORT(void)
LLVMPY_AddLoopUnrollPass_module(LLVMModulePassManagerRef MPM) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(LoopUnrollPass()));
}

API_EXPORT(void)
LLVMPY_LLVMAddLoopRotatePass_module(LLVMModulePassManagerRef MPM) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(createFunctionToLoopPassAdaptor(LoopRotatePass())));
}


API_EXPORT(void)
LLVMPY_LLVMAddInstructionCombinePass_module(LLVMModulePassManagerRef MPM) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(InstCombinePass()));
}


API_EXPORT(void)
LLVMPY_AddJumpThreadingPass_module(LLVMModulePassManagerRef MPM, int T) {
    llvm::unwrap(MPM)->addPass(createModuleToFunctionPassAdaptor(JumpThreadingPass(T)));
}
...
LLVMPY_CreateNewFunctionPassManager() { return llvm::wrap(new PassManager<Function>()); }

API_EXPORT(void)
LLVMPY_NMPRun_function(LLVMFunctionPassManagerRef FPMRef, LLVMPassBuilderRef PBRef, LLVMValueRef FRef) {
...
API_EXPORT(void)
LLVMPY_AddAAEvalPassNFPM(LLVMFunctionPassManagerRef FPM) {
...
LLVMPY_LLVMAddLoopRotatePass_function(LLVMFunctionPassManagerRef FPM) {
    llvm::unwrap(FPM)->addPass(createFunctionToLoopPassAdaptor(LoopRotatePass()));
}
...
LLVMPY_CreatePipelineTuningOptions() { return llvm::wrap(new PipelineTuningOptions()); }
...
LLVMPY_CreatePassBuilder(LLVMTargetMachineRef TM, LLVMPipelineTuningOptionsRef PTO) {
...
LLVMPY_buildPerModuleDefaultPipeline(LLVMPassBuilderRef PBref, int opt_level) {   
    ...
    if(opt_level == 0){
        MPM = new ModulePassManager(PB->buildO0DefaultPipeline(OptimizationLevel::O0));
...
API_EXPORT(LLVMFunctionPassManagerRef)
LLVMPY_buildFunctionSimplificationPipeline(LLVMPassBuilderRef PBref, int opt_level) {   
    ...
    FPM = new FunctionPassManager(PB->buildFunctionSimplificationPipeline(lvl, ThinOrFullLTOPhase::None));
    return llvm::wrap(FPM);
}

API_EXPORT(void)
LLVMPY_DisposePassBuilder(LLVMPassBuilderRef PB){
    delete llvm::unwrap(PB);
}
