...
def run_llvm_config(llvm_config, args):
    ...
    if rc != 0:
        raise RuntimeError(
            "Command %s returned with code %d; stderr follows:\n%s\n" % (cmd, rc, err)
        )
    return out
...
def find_windows_generator():
    ...
    if env_generator is not None:
        ...
        generators.append((env_generator, env_arch, env_toolkit))
...
def main_posix(kind, library_ext):
    ...
    try:
        ...
        msg = (
            "Could not find a `llvm-config` binary. There are a number of "
        ...
        raise RuntimeError(
            "%s failed executing, please point LLVM_CONFIG "
    ...
    if skipcheck:
        ...
        msg = (
            "The LLVM version check for supported versions has been "
            "overridden.\nThis is unsupported behaviour, llvmlite may not "
            "work as intended.\nRequested LLVM version: {}".format(out.strip())
        )
        warn = " * ".join(("WARNING",) * 8)
        blk = "=" * 80
        warning = "{}\n{}\n{}".format(blk, warn, blk)
        ...
        if not out.startswith("11"):
            msg = (
                "Building llvmlite requires LLVM 11.x.x, got "
